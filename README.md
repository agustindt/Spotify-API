...
5. In the Spotify Developer Dashboard, add `http://127.0.0.1:5173` (or another loopback address) to your allowed redirect URIs for local development. **Do not use `localhost` as it is rejected by Spotify.** The value must exactly match `REDIRECT_URI` in `src/auth.ts`. Use `https://` for any non-loopback address.

6. Click **Login with Spotify**. Authorize the requested scopes when prompted:
